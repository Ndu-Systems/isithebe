...
export class EditClientComponent implements OnInit {
    ...
    archive(client) {
        client.StatusId = 2;
        //create a note for the archive table
        let data = {
            ItemId: client.UserId,
            TableName: 'Clients',
            Reason: 'Authorized MVP functionality',
        ...
        this.confirmationService.confirm({
            ...
            accept: () => {
                ...
                this.archiveService.addArchive(data)
                    .subscribe(response => {
                        if (response === 1) {
                            this.showSuccess('Client Archived Successfully', 'warn');
                            setTimeout(() => {
    ...
    update(client) {
        ...
        this.clientService.updateClient(data).subscribe(response => {
            if (response === 1) {
                this.showSuccess('Client Updated Successfully', 'success');
                setTimeout(() => {
                }, 2000);
            }
